...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    // checking if there is a white ball in the image or otherwise the robot will stop
     
    if((left_pixel_count == 0) && (middle_pixel_count == 0) && (right_pixel_count == 0))
    {
        drive_robot(0.0, 0.0);
    }
    ...
    else
    {
        ...
        if((left_pixel_count > right_pixel_count) && (left_pixel_count > middle_pixel_count))
        {
            drive_robot(0.35, 0.35);
        }

        // Go straight if the ball is in the middle part of the image
        else if((middle_pixel_count > left_pixel_count) && (middle_pixel_count > right_pixel_count))
        {
